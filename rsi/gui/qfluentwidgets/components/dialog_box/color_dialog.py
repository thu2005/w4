...
class BrightnessSlider(ClickableSlider):
    ...
    def __onValueChanged(self, value):
        ...
        self.color.setHsv(
            self.color.hue(), self.color.saturation(), value, self.color.alpha()
        )
...
class HexColorLineEdit(ColorLineEdit):
    ...
    def __init__(self, color, parent=None, enableAlpha=False):
        ...
        if enableAlpha:
            self.setValidator(
                QRegularExpressionValidator(QRegularExpression(r"[A-Fa-f0-9]{8}"))
            )
        else:
            self.setValidator(
                QRegularExpressionValidator(QRegularExpression(r"[A-Fa-f0-9]{6}"))
            )
...
class OpacityLineEdit(ColorLineEdit):
    ...
    def __init__(self, value, parent=None, enableAlpha=False):
        ...
        self.setValidator(
            QRegularExpressionValidator(QRegularExpression(r"[0-9][0-9]{0,1}|100"))
        )
        self.setTextMargins(4, 0, 33, 0)
